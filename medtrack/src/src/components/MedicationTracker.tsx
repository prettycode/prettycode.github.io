...
export const MedicationTracker: React.FC = () => {
    ...
    let appUrlPath = window.location.host + window.location.pathname;

    appUrlPath = appUrlPath.endsWith('/') ? appUrlPath.slice(0, -1) : appUrlPath;
    ...
    return (
        <div className="max-w-md mx-auto p-4 bg-gray-50 rounded-lg shadow-md">
            ...
            <p className="text-xs text-center text-gray-600 mt-3">
                &copy; {fullYearCreated}
                {new Date().getFullYear() > fullYearCreated ? '–' + new Date().getFullYear() : ''} {appUrlPath}
            </p>
