    ...
    switch (action.type) {
        ...
        case 'UPDATE_DOSE':
            return {
                ...state,
                doses: state.doses
                    .map(dose => (dose.id === action.payload.id ? { ...dose, time: action.payload.time } : dose))
...
export const MedicationProvider: React.FC<MedicationProviderProps> = ({ children }) => {
    ...
    // Google Auth context
    const { isInitialized, isAuthenticated, backupData, restoreData, isBackupInProgress, lastBackupTime } = useGoogleAuth();
    ...
    const calculateNextDoseTime = useCallback(
        (dosesArray: Dose[] = state.doses) => {
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            ...
            if (now >= state.nextDoseTime) {
                const { dayOfWeek, timeOfDay, timeZone } = dateUtils.formatDateTimeWithDayOfWeek(state.nextDoseTime.toISOString());
    ...
    useEffect(() => {
        ...
        // Backup to Google Drive if authenticated and data changes
        if (isInitialized && isAuthenticated && state.doses.length > 0) {
            // Only backup if it's been more than 5 minutes since the last backup
            const now = new Date();
            if (!lastBackupRef.current || now.getTime() - lastBackupRef.current.getTime() > 5 * 60 * 1000) {
                backupToGoogleDrive();
                lastBackupRef.current = now;
            }
        }
    }, [state.doses, setStoredDoses, isInitialized, isAuthenticated]);
    ...
    useEffect(() => {
        ...
        // Backup to Google Drive if authenticated and interval changes
        if (isInitialized && isAuthenticated) {
            backupToGoogleDrive();
        }
    }, [state.intervalMinutes, calculateNextDoseTime, setStoredInterval, state.doses.length, isInitialized, isAuthenticated]);
    ...
    // Try to restore data from Google Drive when initialized
    useEffect(() => {
        if (isInitialized && isAuthenticated && !isRestoringData && state.doses.length === 0) {
            restoreFromGoogleDrive();
        }
    }, [isInitialized, isAuthenticated, isRestoringData, state.doses.length]);

    // Backup data to Google Drive
    const backupToGoogleDrive = useCallback(async () => {
        if (!isInitialized || isBackupInProgress) {
            return;
        }
        ...
        try {
            ...
            setBackupError((error as Error).message);
        }
    }, [isInitialized, isBackupInProgress, backupData, state.doses, state.intervalMinutes]);

    // Restore data from Google Drive
    const restoreFromGoogleDrive = useCallback(async () => {
        if (!isInitialized || isRestoringData) {
            return;
        }
        ...
        try {
            ...
            if (data) {
                if (data.doses && Array.isArray(data.doses)) {
                    // Replace local doses with Google Drive backup
                    dispatch({ type: 'RESET_DOSES' });
                    data.doses.forEach((dose: Dose) => {
                        dispatch({ type: 'ADD_DOSE', payload: dose });
                    });
                }

                if (data.intervalMinutes && typeof data.intervalMinutes === 'number') {
                    dispatch({ type: 'SET_INTERVAL', payload: data.intervalMinutes });
                }
            }
        } catch (error) {
    ...
    }, [isInitialized, isRestoringData, restoreData]);
    ...
    const getDoseAdherence = useCallback(
        (doseTime: string, doseIndex: number): DoseAdherenceInfo => {
