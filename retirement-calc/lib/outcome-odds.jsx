...
function OutcomeOdds({
  ...
  const inN = (p) => {
    if (p < 0.01 || p > 0.99) return `about ${Math.max(1, Math.round(p * 1000))} in 1,000`;
  ...
  const ranOutBy = (key) => {
    for (let y = 1; y <= simYears; y++) {
      if (yearData[y].endBalance[key] <= 0) return y;
    }
    return null;
  };
  ...
  const lifespanLadder = [
    ...
    return y === null
      ? { odds: r.odds, primary: `still had money at ${horizon}`, median: r.median }
      : { odds: r.odds, lead: 'had run out by', primary: byLabel(y), median: r.median };
  });
  ...
  return (
    <section className="odds-section fade" aria-labelledby="odds-title">
      <h2 className="chart-title" id="odds-title">Reading the Odds</h2>
      <p className="chart-subtitle">
        What the bands above do — and don't — say about how this plan is likely to turn out.
      </p>
      ...
      <p className="odds-lead">
        ...
        {firstTenthGone !== null && ` 1 in 10 had run out by ${byLabel(firstTenthGone)} — the year the outer band above first touches the axis.`}
      </p>
        ...
        {ladder.map((rung, i) => (
          <div className={`odds-rung${rung.median ? ' median' : ''}`} key={i}>
            ...
            <dd>
              ...
              {rung.secondary && <span className="odds-rung-real">{rung.secondary}</span>}
      ...
      {medianSurvives ? (
        <p className="odds-ladder-note">
          Read a rung as the chance of clearing that figure — 1 in 4 futures finish above{' '}
          {fmtMoney(ending.p75)}. Taken in pairs, the rungs give a 1-in-2 chance of finishing between{' '}
          {fmtMoney(ending.p25)} and {fmtMoney(ending.p75)}, and a 4-in-5 chance of finishing{' '}
          {ending.p10 > 0
        ...
        <p className="odds-ladder-note">
          Read a rung as a running total: by {byLabel(ranOutBy('p25'))}, a quarter of all futures had
          run dry. More than half run dry before {horizon}, which is why the median ending balance is
          $0 — each rung is the year the matching band above meets the axis.
        </p>
      )}

      <p className="odds-caveat">
        So the median is the middle outcome, not the most likely one — it is simply the line with half the
        futures above it and half below.
        {medianSurvives
