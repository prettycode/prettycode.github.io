...
const ETFPortfolioVisualizer = () => {
  ...
  // Toggle between builder mode and example portfolios
      ...
      <div className="mb-6">
        <div className="flex justify-between mb-4">
          <label className="block text-sm font-medium mb-2 text-gray-700">
            {isBuilderMode ? "Build Your Portfolio:" : "Select Portfolio:"}
          </label>
          <button
            onClick={toggleBuilderMode}
            className="px-4 py-2 rounded-md bg-gray-800 text-white hover:bg-gray-700 transition-colors"
          >
            {isBuilderMode ? "View Example Portfolios" : "Build My Own Portfolio"}
          </button>
        </div>
        ...
        {!isBuilderMode ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 mb-4">
            <div className="flex flex-wrap justify-between items-center mb-4">
              <div>
                <h3 className="text-lg font-bold">My Custom Portfolio</h3>
                    ...
                    {customPortfolio.holdings.size === 0 && (
                      <tr>
                        <td colSpan={showDetailColumns ? "6" : "3"} className="px-6 py-4 text-center text-sm text-gray-500">
                          No ETFs added yet. Add ETFs from the list above.
                        </td>
                      </tr>
                    )}
                  </tbody>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <div className="mb-6">
          <h2 className="text-xl font-bold text-gray-800">
            {isBuilderMode ? customPortfolio.name : selectedPortfolio.name}
          </h2>
          <p className="text-gray-600">
            {isBuilderMode ? customPortfolio.description : selectedPortfolio.description}
          </p>
          
          {isBuilderMode && customPortfolio.holdings.size === 0 && (
            <div className="mt-4 p-4 bg-gray-100 rounded-md text-gray-700">
              Select ETFs above to start building your portfolio. Your portfolio analysis will appear here.
            </div>
          )}
        </div>
        
        {(!isBuilderMode || (isBuilderMode && customPortfolio.holdings.size > 0)) && (
          <>
            {/* Only show the ETF Allocation table in non-builder mode since it's now part of the builder UI */}
            {!isBuilderMode && <PortfolioCompositionList portfolio={selectedPortfolio} />}
            <AssetClassExposureBar portfolio={isBuilderMode ? customPortfolio : selectedPortfolio} />
            <DetailedExposuresVisual portfolio={isBuilderMode ? customPortfolio : selectedPortfolio} />
          </>
        )}
      </div>
    </div>
