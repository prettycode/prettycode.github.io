...
const PortfolioBuilderG = () => {
  const [customPortfolio, setCustomPortfolio] = useState(
    createPortfolio("My Custom Portfolio", "Build your own portfolio", [])
  );
  ...
  const updateETFAllocation = (ticker, newPercentage) => {
    ...
    const parsedValue = Math.max(
      0,
      Math.min(100, parseFloat(newPercentage) || 0)
    );
    ...
    const lockedAllocation = Array.from(holdings.entries())
      ...
      .reduce(
        (sum, [_, holding]) =>
          sum + (holding.disabled ? 0 : holding.percentage),
        0
      );
  ...
  const saveCustomPortfolio = () => {
    ...
    alert(
      "Portfolio saved! (Implementation would depend on your application's needs)"
    );
  };
                  ...
                  onChange={(e) => {
                    if (e.target.value) {
                      const selectedPortfolio =
                        examplePortfolios[parseInt(e.target.value)];
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                          ...
                          ([ticker, holding], index) => {
                            ...
                            if (etf) {
                              for (const [key, amount] of etf.exposures) {
                                ...
                                constituents.push(
                                  `${description}: ${(amount * 100).toFixed(
                                    1
                                  )}%`
                                );
                              }
                        ...
                        {customPortfolio.holdings.size === 0 && (
                          <tr>
                            ...
                            >
                              No ETFs added yet. Add ETFs from the list above or
                              select a template.
                            </td>
                          </tr>
                        )}
