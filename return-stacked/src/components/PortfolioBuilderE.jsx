...
import { PortfolioBuilderLegacy } from './PortfolioBuilderLegacy';
                ...
                <div className="flex flex-col space-y-6">
                    {Object.entries(EXPOSURE_MAPPING).map(([category, subcategories]) => (
                        <Card key={category} className="shadow-sm">
                            <CardHeader
                                className={`${CATEGORY_COLORS[category].header} flex flex-row justify-between items-center`}
                            >
                                <CardTitle>
                                    {category} Exposure {displayMode[category] ? '(Absolute)' : '(Relative)'}
                                </CardTitle>
