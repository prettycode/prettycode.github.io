...
const scoreBucket = (score: number): { label: string; tone: 'good' | 'mixed' | 'poor' } => {
    if (score >= 8) return { label: 'Efficient', tone: 'good' };
    if (score >= 6) return { label: 'Moderate', tone: 'mixed' };
            ...
            {isExpanded && (
                <CardContent className="px-4 pt-0 pb-3">
                    <div className={cn('text-xs mb-2', tone.text)}>
                        Weighted average across active holdings (excludes expense ratios). Score ranges 1 (least efficient) to 10
                        (most efficient). Most relevant in taxable accounts; in IRAs/401(k)s this matters less.
                    </div>
                                ...
                                {sortedRows.map((row) => {
                                    ...
                                    return (
                                        <tr key={row.ticker} className="border-b">
                                            <td className={cn('p-2 align-top font-medium', rowTone.text)}>{row.ticker}</td>
