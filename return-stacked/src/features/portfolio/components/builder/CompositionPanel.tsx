...
interface CompositionPanelProps {
    ...
    onLoadPortfolio?: (portfolio: Portfolio | SerializedPortfolio) => void;
...
const CompositionPanel: React.FC<CompositionPanelProps> = ({
    ...
    const handleEqualWeightUnlocked = (): void => {
        ...
        const totalLockedOrDisabledBasisPoints = holdings
            ...
            .reduce(
                (sum, [_ticker, holding]) => sum + (holding.basisPoints ?? percentToBasisPoints(holding.percentage)),
                0
            );
    ...
    const isAlreadyEqualWeightUnlocked =
        ...
        ((): boolean => {
            const totalLockedOrDisabledBasisPoints = holdings
                ...
                .reduce(
                    (sum, [_ticker, holding]) =>
                        sum + (holding.basisPoints ?? percentToBasisPoints(holding.percentage)),
                    0
                );
                    ...
                    {showPortfolioSelector && onLoadPortfolio && examplePortfolios && savedPortfolios && (
                        <div className="space-y-2">
                            <TickerOrTemplateSelectionTable
                                ...
                                onSelect={(item) => {
                                    onLoadPortfolio(item as Portfolio | SerializedPortfolio);
