...
import type { Portfolio, SerializedPortfolio, Holding } from '@/features/portfolio/core/domain/Portfolio';
...
interface BuilderProps {
    ...
    onUpdatePortfolio: (
        ...
        originalTemplate?: Portfolio | null
    ) => void;
...
const Builder: React.FC<BuilderProps> = ({
    ...
    const loadPortfolio = (portfolio: Portfolio | SerializedPortfolio): void => {
        const isSaved = Array.isArray(portfolio.holdings);
        const portfolioToLoad = isSaved
            ? deserializePortfolio(portfolio as SerializedPortfolio)
            : (portfolio as Portfolio);
        const newHoldings = new Map<string, Holding>(portfolioToLoad.holdings);
        ...
        onUpdatePortfolio(
            ...
            !isSaved,
            !isSaved ? (portfolio as Portfolio) : null
        );
    ...
    return (
        <div className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground mb-4">
                Change the portfolio composition using the controls below:
            </p>
            ...
            {/* Introduction Message */}
            {
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                        Add ETFs to your portfolio by selecting them from the table below.
                    </p>
                    {/* Template and Saved Portfolios Selection Table */}
                    {
                        <TickerOrTemplateSelectionTable
